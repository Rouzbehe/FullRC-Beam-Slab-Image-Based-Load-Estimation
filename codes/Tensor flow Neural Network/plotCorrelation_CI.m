...
nameFolder='Mar_7_10CV_on_specimen_All beams W and WO_TensorflowNN';
...
elseif Plot_No==32
  ...
  ylabel('Predicted scaled moment at mid span  (M/(abh (l-a)))','fontsize',size); 
    ...
    elseif Plot_No==40
  xlabel('True scaled moment at mid span ($$\frac{M}{\rho~EI}$$)','fontsize',sizeLATEX,'FontName','Helvetica','Interpreter','latex');
  h=ylabel('Predicted scaled moment at mid span ($$\frac{M}{\rho~EI}$$)','fontsize',sizeLATEX,'FontName','Helvetica','Interpreter','latex');
       NameJPG2 = strcat(nameFolder,'/',plotfname,'ConfidenceInterval','M_rho_EI');
...
%  ylabel('Predicted scaled shear     ','fontsize',size);  
  ...
  ylabel('Predicted  Failure ratio (M/M_{failure}) or (V/V_{failure})','fontsize',size); 
      ...
      elseif Plot_No==44
  xlabel('True scaled shear $$(V/(2\sqrt{f_c}~bd+A_v~f_y~d/s)$$','fontsize',10,'FontName','Helvetica','Interpreter','latex');
  h=ylabel('Predicted  scaled shear $$(V/(2\sqrt{f_c}~bd+A_v~f_y~d/s)$$','fontsize',10,'FontName','Helvetica','Interpreter','latex');
  
                        NameJPG2 = strcat(nameFolder,'/',plotfname,'ConfidenceInterval','Failure Ratio');                        
      elseif Plot_No==46
  xlabel('True shear strength of reinforcing steel $$(V_s=A_v f_y d/s)$$','fontsize',size);
  ylabel('Predicted  shear strength of reinforcing steel  $$(V_s=A_v f_y d/s)$$','fontsize',size); 
  
                        NameJPG2 = strcat(nameFolder,'/',plotfname,'ConfidenceInterval','Failure Ratio');                               
      elseif Plot_No==47
  xlabel('True shear index ($$\frac{A_v~f_y}{d~s}$$)','fontsize',sizeLATEX,'FontName','Helvetica','Interpreter','latex');
  h=ylabel('Predicted shear index ($$\frac{A_v~f_y}{d~s}$$)','fontsize',sizeLATEX,'FontName','Helvetica','Interpreter','latex');
  
                        NameJPG2 = strcat(nameFolder,'/',plotfname,'ConfidenceInterval','shearindex');          
       elseif Plot_No==48
   xlabel('True scaled moment at mid span   (M/(\rho bd)) ','fontsize',size);
  ylabel('Predicted scaled moment at mid span  (M/(\rho bd)) ','fontsize',size);  
...
xlabel('Prediction Interval (%)','fontsize',size,'Interpreter','tex');
ylabel('Average margin of error','fontsize',size,'Interpreter','tex'); 
set(gca,'fontsize',size)
% print('-dpdf',char(Namepdf),'-r300')
